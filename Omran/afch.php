...
if(isset($_GET['typop']))
{
	...
	if($type=="cl")
	{
		...
		$req=mysql_query("select count(*) nb from evenement where date between '".$y.'-'.$m.'-'."01' and '".$y.'-'.$m.'-'."31' and etat in('approuve','annule','realise')") or die(mysql_error());
		...
		if($nb!=0)
		{
			$reqt=mysql_query("select * from evenement where date between '".$y.'-'.$m.'-'."01' and '".$y.'-'.$m.'-'."31' and etat in('approuve','annule','realise') order by date asc") or die(mysql_error());
