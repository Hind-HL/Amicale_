...
if(isset($_GET['typop']))
{
	...
	if($type=="cl")
	{
		...
		$req=mysql_query("select count(*) nb from evenement where date between '".$y.'-'.$m.'-'."01' and '".$y.'-'.$m.'-'."31' and (etat='approuve' or etat ='realise' or etat='annule')") or die(mysql_error());
		...
		if($nb!=0)
		{
			$reqt=mysql_query("select * from evenement where date between '".$y.'-'.$m.'-'."01' and '".$y.'-'.$m.'-'."31' and (etat='approuve' or etat ='realise' or etat='annule') order by date asc") or die(mysql_error());
		...
		?>
            ...
            <br />
            <div style="height:40px;width:90%;margin-left:auto;margin-right:auto"><div style="float:left"><span style="height:20px;width:30px;display:block;border:1px solid black;background-color:rgba(0,230,57,1);float:left"></span>&nbsp;: Evenement réalisé.</div><div style="float:left;margin-left:20%"><span style="height:20px;width:30px;display:block;border:1px solid black;background-color:blue;float:left"></span>&nbsp;: Evenement approuvé.</div><div style="float:left;margin-left:20%"><span style="height:20px;width:30px;display:block;border:1px solid black;background-color:red;float:left"></span>&nbsp;: Evenement annulé.</div></div>
		<?php
		echo '****'.$m;
